"""준비된 노트를 LiteLLM으로 순차 호출한다."""

# 출처: src/verifiable_ai_workflow/providers/litellm_provider.py — 복사.
# 차이: 구조화 출력 스키마를 StructuredAnswer에서 RelationVerdict로 바꿨다.
# 이미지가 아닌 텍스트 메시지를 받는다는 점 말고는 호출 규율이 같다.
...
from ..schemas import RelationVerdict
        ...
        if self.structured_output == "json_schema":
            request["response_format"] = {
                ...
                "json_schema": {
                    ...
                    "strict": True,
                    "schema": RelationVerdict.model_json_schema(),
                },
            }
        response = None
